...
/**
 * @author yuzhuliu:
 * @version 创建时间：2017年5月26日 上午11:07:15
 * 类说明 远程调用Linux shell 命令
 */
public class EXEShell {
	    ...
	    private static boolean connect(String user, String passwd, String host) {  
	        ...
	        try {  
	            ...
	            e.printStackTrace();  
	            System.out.println("connect error !连接错了");  
	            return false;  
	        }  
	        return true;  
	    }  
	    ...
	    public static Map<String, String> runDistanceShellN(String command, String user, String passwd, String host) {  
	        if (!connect(user, passwd, host)) {  
	            return null;  
	        }  
	        Map<String, String> map = new HashMap<>();  
	        ...
	        try {  
	                ...
	                while ((buf = reader.readLine()) != null) {  
	                    ...
	                    stringBuffer.append(buf.trim().toString().replaceAll("\\r\\n","<br/>"));  
	                //每个命令存储自己返回数据-用于后续对返回数据进行处理  
	                map.put(command, stringBuffer.toString());  
	            }  
	        } catch (IOException | JSchException e) {  
