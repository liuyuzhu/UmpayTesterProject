...
import com.umpaytester.spay.util.SYTBean;
import com.umpaytester.spay.util.SignForSYT;
import com.umpaytester.utils.GetSms;

public class cb_api_refund {
	
	public static void main(String[] args) throws UnsupportedEncodingException {
		cb_api_refund test = new cb_api_refund();
		test.Test_cb_api_refund();
	}
	@Test
	 
	 public void Test_cb_api_refund() throws UnsupportedEncodingException{
...
//		 System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
		 if(RetMsg.contains("&ret_code=0000&ret_msg=操作成功")){
			 ...
			 System.out.println(String.format("[测试接口1"+this.getClass().getSimpleName()+"----调用接口----"+SYTBean.getService()+"--失败")+RetMsg);
			 break;
...
//   	 System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
   	 RetMsg=SignForSYT.sign(request, tb.url);
//	 System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
	 if(RetMsg.contains("&ret_msg=原交易状态不正确,不允许退费")){
		 System.out.println(String.format("[测试接口1"+this.getClass().getSimpleName()+"----调用接口----"+SYTBean.getService()+"--成功"));
	 }else{
		 System.out.println(String.format("[测试接口1"+this.getClass().getSimpleName()+"----调用接口----"+SYTBean.getService()+"--失败"));
		 break;
...
//	 System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
//     assertTrue(RetMsg.contains("&ret_msg=财务处理中")); 
}
}
