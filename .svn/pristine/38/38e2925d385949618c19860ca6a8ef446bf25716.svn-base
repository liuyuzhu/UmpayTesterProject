...
function check(){
	...
	//	alert(a);
		if(a =="NOT_APPLICABLE"){
			document.getElementById("interface_type").value = "SERVER_TO_WEB";
		}else if(a =="NOT_APPLICABLEH5"){
			document.getElementById("interface_type").value = "SERVER_TO_H5WEB";
			document.getElementById("payment_method").value = "NOT_APPLICABLE";
		}else{
			
			document.getElementById("interface_type").value = "SERVER_TO_SERVER";
...
<script type="text/javascript">
    $(function(){
        ...
        $("#testAjax").click(function(){
            ...
            $.ajax({
               type: "POST",
               url: "/UmpayTesterProject/SpayRestOrderApiServlet",
               data: data,
...
<div class="div-left"  style="height: 570px;width: 489px;  font-size:12px; background: white; color: #aaa; padding: 20px;border:2px solid  #242424">
	...
	<table  style="width: 500px; height: 150px;" border="0" cellpadding="1" cellspacing="1">
			...
			<td  >环境选择<label style="color: red">*</label></td>
					...
					<td style="height: 30px;" >
                       ...
                       <option  value="CREDIT_CARD"   selected="selected" >快捷支付信用卡</option>
						<option  value="DEBIT_CARD"  >快捷支付借记卡</option>
						<option  value="CREDIT_CARD"  >协议支付信用卡</option>
						<option  value="DEBIT_CARD"  >协议支付借记卡</option>
						<option   value="WECHAT_SCAN" >融合支付微信主扫</option>
						<option  value="ALIPAY_SCAN" >融合支支付宝主扫</option>
						<option  value="WECHAT_IN_APP" >融合支微信APP支付</option>
						<option  value="WECHAT_WEB" >融合支微信公众号支付</option>
						<option  value="NOT_APPLICABLEH5"  >收银台H5下单</option>
						<option  value="NOT_APPLICABLE" >收银台下单</option>
   ...
   <td style="height: 30px;" valign="middle">商品信息描述</td><td><input style="width: 150px; height: 100%;border:5px solid blue;padding:5px;background-color:transparent;"  type=text name="goods_inf" id="goods_inf" value="联动优势"/></td>
		...
		<tr>
					...
					<td style="height: 30px;" valign="middle">银行编码</td><td><input style="width: 150px; height: 100%;border:5px solid blue;padding:5px;background-color:transparent;" type=text name="bank_code" id="bank_code"  value="BOC"  /></td>
	...
	<td style="height: 30px;" valign="middle">子订单号</td><td><input style="width: 150px; height: 100%;border:5px solid blue;padding:5px;background-color:transparent;" type=text name="mer_sub_reference_id"  id="mer_sub_reference_id"  value="<%=orderId.substring(4) %>2" /></td>
	</tr><tr>
